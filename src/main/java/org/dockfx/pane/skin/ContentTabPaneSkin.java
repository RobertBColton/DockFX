...
/**
 * This class source is copied from com.sun.javafx.scene.control.skin.TabPaneSkin in jfxrt.jar 1.8
 * version In the different java version, this source might not be working properly. The inner class
 * TabMenuItem is modified to handle DockNodeTab title because the tab uses customized graphics
 */
public class ContentTabPaneSkin extends BehaviorSkinBase<TabPane, TabPaneBehavior> {
  ...
  private ObjectProperty<TabAnimation> openTabAnimation =
      new StyleableObjectProperty<TabAnimation>(TabAnimation.GROW) {
        @Override
  ...
  /**
   * VERY HACKY - this lets us 'duplicate' Label and ImageView nodes to be used in a Tab and the
   * tabs menu at the same time.
   */
  ...
  public ContentTabPaneSkin(TabPane tabPane) {
    ...
    if (selectedTab == null && getSkinnable().getSelectionModel().getSelectedIndex() != -1) {
      getSkinnable().getSelectionModel().select(
          getSkinnable().getSelectionModel().getSelectedIndex());
      selectedTab = getSkinnable().getSelectionModel().getSelectedItem();
    }
    if (selectedTab == null) {
  ...
  private Timeline createTimeline(final TabHeaderSkin tabRegion, final Duration duration,
      final double endValue, final EventHandler<ActionEvent> func) {
  ...
  protected double computePrefWidth(double height, double topInset, double rightInset,
    ...
    final double tabHeaderAreaSize =
        snapSize(isHorizontal ? tabHeaderArea.prefWidth(-1) : tabHeaderArea.prefHeight(-1));
  ...
  protected double computePrefHeight(double width, double topInset, double rightInset,
    ...
    final boolean isHorizontal = isHorizontal();
    final double tabHeaderAreaSize =
        snapSize(isHorizontal ? tabHeaderArea.prefHeight(-1) : tabHeaderArea.prefWidth(-1));

    double prefHeight =
        isHorizontal ? maxh + snapSize(tabHeaderAreaSize) : Math.max(maxh, tabHeaderAreaSize);
    return snapSize(prefHeight) + topInset + bottomInset;
  }

  @Override
  public double computeBaselineOffset(double topInset, double rightInset, double bottomInset,
      double leftInset) {
    Side tabPosition = getSkinnable().getSide();
  ...
  class TabHeaderArea extends StackPane {
        ...
        protected void layoutChildren() {
          if (tabsFit()) {
            ...
            if (!removeTab.isEmpty()) {
              double offset = 0;
              double w =
                  tabHeaderArea.getWidth() - snapSize(controlButtons.prefWidth(-1))
          ...
          double tabX =
              (tabPosition.equals(Side.LEFT) || tabPosition.equals(Side.BOTTOM)) ? snapSize(getWidth())
                  - getScrollOffset()
                  : getScrollOffset();
          ...
          for (Node node : getChildren()) {
            ...
            double tabHeaderPrefWidth =
                snapSize(tabHeader.prefWidth(-1) * tabHeader.animationTransition.get());
            ...
            double startY =
                tabPosition.equals(Side.BOTTOM) ? 0 : tabBackgroundHeight - tabHeaderPrefHeight
                    - snappedBottomInset();
    ...
    private void ensureSelectedTabIsVisible() {
      ...
      double tabPaneWidth =
          snapSize(isHorizontal() ? getSkinnable().getWidth() : getSkinnable().getHeight());
    ...
    protected double computePrefWidth(double height) {
      double padding =
          isHorizontal() ? snappedLeftInset() + snappedRightInset() : snappedTopInset()
              + snappedBottomInset();
    ...
    protected double computePrefHeight(double width) {
      double padding =
          isHorizontal() ? snappedTopInset() + snappedBottomInset() : snappedLeftInset()
              + snappedRightInset();
    ...
    protected void layoutChildren() {
      ...
      double w =
          snapSize(getWidth()) - (isHorizontal() ? leftInset + rightInset : topInset + bottomInset);
      double h =
          snapSize(getHeight())
              - (isHorizontal() ? topInset + bottomInset : leftInset + rightInset);
      double tabBackgroundHeight = snapSize(prefHeight(-1));
      double headersPrefWidth = snapSize(headersRegion.prefWidth(-1));
      double headersPrefHeight = snapSize(headersRegion.prefHeight(-1));
      ...
      if (headerBackground.isVisible()) {
        positionInArea(headerBackground, 0, 0, snapSize(getWidth()), snapSize(getHeight()), /*
                                                                                             * baseline
                                                                                             * ignored
                                                                                             */0,
            HPos.CENTER, VPos.CENTER);
      }
      positionInArea(headersRegion, startX, startY, w, h, /* baseline ignored */0, HPos.LEFT,
          VPos.CENTER);
  ...
  class TabHeaderSkin extends StackPane {
    ...
    public TabHeaderSkin(final Tab tab) {
      ...
      inner = new StackPane() {
        ...
        protected void layoutChildren() {
          ...
          // provided max width.
          double closeBtnStartX =
              (maxWidth < Double.MAX_VALUE ? Math.min(w, maxWidth) : w) - paddingRight
                  - closeBtnWidth;
          ...
          focusIndicator.resizeRelocate(paddingLeft - hPadding, paddingTop + vPadding, w + 2
              * hPadding, h - 2 * vPadding);
        }
      };
    ...
    private void updateGraphicRotation() {
      if (label.getGraphic() != null) {
        label.getGraphic().setRotate(
            getSkinnable().isRotateGraphic() ? 0.0F
                : (getSkinnable().getSide().equals(Side.RIGHT) ? -90.0F : (getSkinnable().getSide()
                    .equals(Side.LEFT) ? 90.0F : 0.0F)));
      }
    }

    private boolean showCloseButton() {
      return tab.isClosable()
          && (getSkinnable().getTabClosingPolicy().equals(TabClosingPolicy.ALL_TABS) || getSkinnable()
              .getTabClosingPolicy().equals(TabClosingPolicy.SELECTED_TAB) && tab.isSelected());
    }

    private final DoubleProperty animationTransition = new SimpleDoubleProperty(this,
    ...
    protected void layoutChildren() {
      double w =
          (snapSize(getWidth()) - snappedRightInset() - snappedLeftInset())
  ...
  private static final PseudoClass SELECTED_PSEUDOCLASS_STATE = PseudoClass
  ...
  private static final PseudoClass DISABLED_PSEUDOCLASS_STATE = PseudoClass
      .getPseudoClass("disabled");
  ...
  class TabContentRegion extends StackPane {
    ...
    private WeakInvalidationListener weakTabContentListener = new WeakInvalidationListener(
        tabContentListener);
    private WeakInvalidationListener weakTabSelectedListener = new WeakInvalidationListener(
        tabSelectedListener);
      ...
      inner = new StackPane() {
        ...
        protected double computePrefWidth(double height) {
          ...
          double maxArrowWidth =
              !isShowTabsMenu() ? 0 : snapSize(downArrow.prefWidth(getHeight()))
        ...
        private void positionArrow(Pane btn, StackPane arrow, double x, double y, double width,
            double height) {
          btn.resize(width, height);
          positionInArea(btn, x, y, width, height, /* baseline ignored */0, HPos.CENTER,
              VPos.CENTER);
  ...
  class TabMenuItem extends RadioMenuItem {
    ...
    private WeakInvalidationListener weakDisableListener = new WeakInvalidationListener(
        disableListener);
